...
def initParams():
    ...
    # Data folder prepare
    parser.add_argument("-a", "--access_type", type=str, help="LA or PA", default='PA')
    parser.add_argument("-f", "--path_to_features", type=str, help="features path",
                        default='/nfs/home/zhaoyiyang/Data/python_extract/')
    parser.add_argument("-p", "--path_to_protocol", type=str, help="protocol path",
                        default='/nfs/data/raid01/public/')
    ...
    parser.add_argument('--batch_size', type=int, default=180, help="Mini batch size for training")
...
def train(args):
    ...
    if args.continue_training:
        lfcc_model = torch.load(os.path.join(args.out_fold, 'anti-spoofing_mfcc_model.pt')).to(args.device)

    lfcc_optimizer = torch.optim.Adam(lfcc_model.parameters(), lr=args.lr,
                                      betas=(args.beta_1, args.beta_2), eps=args.eps, weight_decay=0.0005)

    training_set = DATA(args.access_type, args.path_to_features, args.path_to_protocol, 'train',
                                'MFCC', feat_len=args.feat_len, padding=args.padding)
    validation_set = DATA(args.access_type, args.path_to_features, args.path_to_protocol, 'dev',
                                  'MFCC', feat_len=args.feat_len, padding=args.padding)
    ...
    for epoch_num in tqdm(range(args.num_epochs)):
        ...
        if val_eer < prev_eer:
            ...
            torch.save(lfcc_model, os.path.join(args.out_fold, 'model.pt'))
            if args.add_loss == "ocsoftmax":
                loss_model = ocsoftmax
                torch.save(loss_model, os.path.join(args.out_fold, 'loss_model.pt'))
            elif args.add_loss == "amsoftmax":
                loss_model = amsoftmax_loss
                torch.save(loss_model, os.path.join(args.out_fold, 'loss_model.pt'))
            else:
...
if __name__ == "__main__":
    args = initParams()
    _, _ = train(args)
    model = torch.load(os.path.join(args.out_fold, 'model.pt'))
    if args.add_loss == "softmax":
        loss_model = None
    else:
        loss_model = torch.load(os.path.join(args.out_fold, 'model.pt'))
